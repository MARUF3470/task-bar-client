...
const MyTask = () => {
    ...
    if (isLoading) {
        return <div role="status" className='text-center my-64'>
    ...
    return (
        <div>
            <h4 className='text-center text-3xl underline my-8'>Your all tasks</h4>
            {
                tasks.length === 0 && <p className='text-center my-52 text-6xl'>No Task Available</p>
            }
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-3 w-11/12 mx-auto'>

                {
                    tasks.length && tasks.map(task => <Task key={task._id} handleDelete={handleDelete} task={task} refetch={refetch} ></Task>
                    )
                }
            </div>
        </div>
